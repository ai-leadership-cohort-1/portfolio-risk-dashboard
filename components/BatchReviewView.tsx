...
export default function BatchReviewView() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10 sm:py-14">
      ...
      <div className="print:hidden flex flex-col gap-4 rounded-lg border border-dashed border-border bg-surface p-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-3">
          <button
            type="button"
            onClick={handleDownloadTemplate}
            className="rounded-md border border-border bg-background px-4 py-2 text-sm font-medium text-foreground shadow-sm hover:bg-border/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40"
          >
          ...
          <button
            ...
            className="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground shadow-sm hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:ring-offset-2"
          >
            Upload CSV
          </button>
          <input
            ...
            {rows.length > 0 && (
              <>
                ...
                {errorCount > 0 && (
                  <>
                    {", "}
                    <span className="font-medium text-red-700">{errorCount} with errors</span>
                  </>
                )}
              </>
            )}
